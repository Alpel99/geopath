...
sys.setrecursionlimit(1500)                                                     # Um "maximum recursion depth exceeded" Error zu vermeiden.
...
with open(file_base + '_path.kml', 'w') as kml:
    ...
    for (x, y, z), coord in zip(path, path_wgs84):
        flight_alt = grid.get_node_value((x, y)) + 110 + z                      # Höhe über Grund = 110m +/- z
        kml.write(f"          {coord[1]},{coord[0]},{flight_alt}\n")            # Höhe über Grund wird in Meereshöhe umgewandelt, Grid-Koordinaten werden in Längen- und Breitengrad umgewandelt
    kml.write( '        </coordinates>\n')
    kml.write( '      </LineString>\n')
    kml.write( '    </Placemark>\n')
    kml.write( '  </Document>\n')
